...
export default function ChatPage() {
    ...
    if (!chatData.members.includes(user.uid)) {
         return <div className="flex h-screen w-full items-center justify-center">Accès refusé.</div>;
    }
                ...
                {messages.map((message, index) => {
                    ...
                    return (
                        <div key={message.id || index} className={`flex ${isSender ? 'justify-end' : 'justify-start'}`}>
                            <div className={`p-3 rounded-lg max-w-sm ${isSender ? 'bg-primary text-primary-foreground' : 'bg-card'}`}>
